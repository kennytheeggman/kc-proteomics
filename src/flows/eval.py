...
def eval(config: Config, model: Model, train_dataloader: DataLoader[Peptide], eval_dataloader: DataLoader[Peptide], global_step, idx, batch, writer):
    model.eval()
    # writer = SummaryWriter(log_dir="runs/mar-26-celena")
    total_sequences = 0
    correct_sequences = 0
    total_tokens = 0
    correct_tokens = 0
    ...
    with torch.no_grad():
        # for idx, batch in enumerate(eval_dataloader):
            ...
            writer.add_scalar("Correct Sequences (%)", p_correct_seqs, idx)
            writer.add_scalar("Correct Tokens (%)", p_correct_tokens, idx)
