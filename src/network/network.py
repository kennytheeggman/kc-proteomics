...
class Model(nn.Module):
    ...
    def forward(self, spectrum: tuple[torch.Tensor, torch.Tensor], sequence: list[str]):
        ...
        target = self.encode_sequence.forward((encoded.to(self.config.device), mask.to(self.config.device)))
        logits = self.decode_sequence.forward((target, mask.to(self.config.device)), embedding)
        # return logits
